<div class="grid grid-cols-4 grid-rows-3 sm:grid-rows-2 gap-x-2 mb-28 sm:mb-32">
    <div class="col-span-4">
        <a href="/MichaelHayworth_Resume.pdf" target="_blank" class="block text-center w-full bg-sky-700 hover:bg-sky-500 focus:bg-sky-500 transition-colors duration-500 text-white font-bold px-6 py-3 rounded-lg"> Resume <i class="fa-solid fa-download"></i></a>
    </div>
    <a href="https://www.linkedin.com/in/michael-hayworth-71a1791a1/" target="_blank" aria-label="LinkedIn Profile"
       class="flex items-center justify-center text-white hover:text-sky-100 focus:text-sky-100 hover:bg-sky-700 focus:bg-sky-700 col-span-2 sm:col-span-1 rounded-lg transition-colors duration-500 h-24">
        <i class="fa-brands fa-linkedin-in fa-2x"></i>
    </a>
    <a href="https://www.freecodecamp.org/fccb870e5bd-2151-4b37-a56c-1d9c107832cd" target="_blank"
       aria-label="freeCodeCamp Profile"
       class="flex items-center justify-center text-white hover:text-sky-100 focus:text-sky-100 hover:bg-sky-700 focus:bg-sky-700 col-span-2 sm:col-span-1 rounded-lg transition-colors duration-500">
        <i class="fa-brands fa-free-code-camp fa-2x"></i>
    </a>
    <a href="https://github.com/andrew-hayworth22" target="_blank" aria-label="GitHub Profile"
       class="flex items-center justify-center text-white hover:text-sky-100 focus:text-sky-100 hover:bg-sky-700 focus:bg-sky-700 col-span-2 sm:col-span-1 rounded-lg transition-colors duration-500">
        <i class="fa-brands fa-github fa-2x"></i>
    </a>
    <a href="https://www.instagram.com/andyhayworth22/?next=%2F" target="_blank" aria-label="Instagram Profile"
       class="flex items-center justify-center text-white hover:text-sky-100 focus:text-sky-100 hover:bg-sky-700 focus:bg-sky-700 col-span-2 sm:col-span-1 rounded-lg transition-colors duration-500">
        <i class="fa-brands fa-instagram fa-2x"></i>
    </a>
</div>
